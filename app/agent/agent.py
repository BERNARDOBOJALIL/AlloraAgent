...
def extract_and_save(state: AlloraState) -> Dict[str, Any]:
    ...
    try:
        ...
        # If model output is plain text (not strict JSON), preserve it as the
        # assistant message so the conversation can continue naturally.
        cleaned_raw = re.sub(r"```(?:json)?", "", raw).strip().rstrip("```").strip()
        assistant_fallback = cleaned_raw or (
            "Hey! Something went a little sideways on my end. "
    ...
    # Keep follow-up flow active until profile is complete, unless user wants to stop.
...
def build_graph() -> Any:
    ...
    checkpointer = MemorySaver()  # short-term: per thread_id
